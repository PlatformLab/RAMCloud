...
echo "#"
echo "# numIndexlets  throughput(khashes/sec)     (kobjs/sec)   (avg latency us)     (numClients for max kobj/sec)" | tee -a "$LOG_DIR/console.log"
...
for (( i=$MIN_INDEXLET; i<=$MAX_INDEXLET; i++ )) do
  ...
  (( SERVERS=$SERVERS_PER_INDEXLET*i+4 ))
  ...
  grep -v '^#' logs/latest/client1\.* | grep -v '/.' | awk -v var="$i" '$2>x{x=$2};$3>y{y=$3;z=1000/($3/$1);a=$1};END{print "\t"var"\t\t"  x"\t"  y"\t" z"\t" a}' | tee -a "$LOG_DIR/console.log"
