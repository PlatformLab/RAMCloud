...
(( CONCURRENT_OPS=4 )) # 30,20,4

# Number of master servers per indexlet
(( SERVERS_PER_INDEXLET=2 )) # 1,2,2
...
PART_TYPE="Hash" # "Range", "Range", "Hash"
...
for (( i=$MIN_INDEXLET; i<=$MAX_INDEXLET; i++ )) do
  ...
  if [[ "$PART_TYPE" = "Hash" ]]
    then
    grep -v '^#' logs/latest/client1\.* | grep -v '/.' | awk -v var="$i" -v numServ="$SERVERS" '$2>T{T=$2;lat=$3;clients=$1};END{print "\t"var"\t"numServ "\t"  T"\t" lat"\t" clients}' | tee -a "$LOG_DIR/console.log"
  else
    grep -v '^#' logs/latest/client1\.* | grep -v '/.' | awk -v var="$i" -v numServ="$SERVERS" '$2>hashesT{hashesT=$2};$T>y{T=$3;lat=$4;clients=$1};END{print "\t"var"\t"numServ "\t"  hashesT"\t"  T"\t" lat"\t" clients}' | tee -a "$LOG_DIR/console.log"
  fi
