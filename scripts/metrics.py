...
"""
This program serves two purposes.  First, it defines all of the RAMCloud
metrics; when run with the --build-only option, this program generates
Metrics.in.cc and Metrics.in.h, which define the available metrics for
RAMCloud servers.  Second, this program is run by various benchmarks to
scan the log files generated by a RAMCloud recovery, extract performance
metrics, and print a summary of interesting data from those metrics.
Eventually, these two functions should probably be separated.
"""
...
class Struct(object):
    ...
    def dumpHeader(self, out, name=''):
        ...
        out('/// %s' % self.doc)
        metricsInitializer = ''
        if self.typeName != 'Metrics':
            out('struct %s {' % self.typeName)
        else:
            metricsInitializer = 'activeCount(0)\n%s, mutex()\n%s, ' % (
                indent, indent)
...
def writeBuildFiles(definitions):
    ...
    cc('Metrics::MetricInfo Metrics::metricInfo(int i)\n{')
    cc('    switch (i) {')
