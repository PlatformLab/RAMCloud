...
writeRandomMultipleAndSync()
{
    ...
    cluster->write(dataTable, key[0], keyLength, value[0], objectSize,
                   NULL, NULL, asyncReplication);
    ...
    printf("# numWrites      Latency       90        99       99.9      "
           "99.99\n");
    ...
    for (int numWrites = 1; numWrites < maxWrites; ++numWrites) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            uint64_t start = Cycles::rdtsc();
            for (int wi = 0; wi < numWrites; wi++) {
                cluster->write(dataTable, key[wi], keyLength, value[wi],
        ...
        printf("%12u %16.1f %16.1f %16.1f %16.1f %16.1f\n", numWrites,
