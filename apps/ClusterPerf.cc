...
// Total number of masters that will be participating in this test.
// Used by writeClusterThroughput test.
static int numMasters;

static int numWitnesses;
...
class WorkloadGenerator {
  public:
        ...
        , generator()
    {
        ...
        generator.construct(recordCount, 0.99);
    }
...
//                    loadGenerator.recordSizeB);
            cluster->write(dataTable, key, keyLen, value,
                    loadGenerator.recordSizeB, NULL, NULL, asyncReplication);
            stop = Cycles::rdtsc();
            if (type == WRITE_TYPE) {
...
// This benchmark measures total throughput of a single server (in objects
// read per second) under a workload consisting of individual random object
// reads. During this benchmark the server of being tested will also serves as
// an witness for another master.
void
readThroughputWhileBeingWitness()
{
    const uint16_t keyLength = 30;
    int size = objectSize;
    if (size < 0)
    ...
    if (clientIndex == 0) {
        ...
        // This client keep write on control table.
        int numKeys = 2000000;
        const uint16_t keyLength = 30;
        char key[keyLength] = "ForWitness";
        char value[objectSize];
        ...
        while (true) {
            ...
            if (running) {
                double totalTime = Cycles::toSeconds(Cycles::rdtsc()
                        - startTime);
                double rate = writeOps/totalTime;
                RAMCLOUD_LOG(NOTICE, "Write rate: %.1f kops/sec", rate/1e03);
            }
            getCommand(command, sizeof(command), false);
            if (strcmp(command, "run") == 0) {
                if (!running) {
                    ...
                    running = true;
                    RAMCLOUD_LOG(NOTICE,
                            "Starting readThroughput benchmark (writer)");
                }
                ...
                uint64_t checkTime = startTime + Cycles::fromSeconds(1.0);
                do {
                    makeKey(downCast<int>(generateRandom() % numKeys),
                            keyLength - 10, key + 10);
                    Util::genRandomString(value, objectSize);
                    // Do the benchmark
                    cluster->write(controlTable, key, keyLength, value,
                                   objectSize, NULL, NULL, true); //Always async
                    ++writeOps;
                } while (Cycles::rdtsc() < checkTime);
            } else if (strcmp(command, "done") == 0) {
                setSlaveState("done");
...
writeRandomMultipleAndSync()
{
    ...
    if (clientIndex != 0) {
        // Continuously send write to server.
        bool running = false;
        uint64_t startTime;
        uint64_t tid;
        ...
        while (true) {
            ...
            if (strcmp(command, "run") == 0) {
                if (!running) {
                    setSlaveState("running");
                    running = true;
                    RAMCLOUD_LOG(NOTICE, "Starting slave");
                    tid = cluster->getTableId("spannedData");
                }
                ...
                do {
                    ...
                    makeKey(downCast<int>(generateRandom() % numObjects),
                            keyLength, key);
                    Util::genRandomString(value, objectSize);
                    cluster->write(tid, key, keyLength, value, objectSize,
                            NULL, NULL, asyncReplication);
                } while (Cycles::rdtsc() < checkTime);
            } else if (strcmp(command, "done") == 0) {
                setSlaveState("done");
                RAMCLOUD_LOG(NOTICE, "Ending slave");
                return;
            } else {
                RAMCLOUD_LOG(ERROR, "unknown command %s", command);
                return;
        ...
        RAMCLOUD_LOG(ERROR, "Entered unreachable code.");
    }

    char key[maxWrites][keyLength];
    char value[maxWrites][objectSize];

    cluster->createTable("spannedData", txSpan);
    uint64_t tid = cluster->getTableId("spannedData");
    fillTable(tid, numKeys, keyLength, objectSize);

    Cycles::sleep(100000);

    // Obtain client lease & everything...
    makeKey(downCast<int>(generateRandom() % numKeys), keyLength, key[0]);
    Util::genRandomString(value[0], objectSize);
    cluster->write(tid, key[0], keyLength, value[0], objectSize,
                   NULL, NULL, asyncReplication);
    ...
    printf("# RAMCloud CGAR-C writes followed by sync benchmark\n");
    printf("# Generated by 'clusterperf.py writeRandomMultipleAndSync'\n#\n");
    printf("#                Latency             90              99"
           "             99.9           99.99\n");
    printf("# numWrites      Median (us)         percentile\n");
    printf("#--------------------------------------------------------"
           "------------------------------\n");


    for (int numWrites = 1; numWrites < maxWrites; ++numWrites) {
        std::vector<uint64_t> ticks;
        ticks.resize(count);
        ...
        for (int i = 0; i < count; i++) {
            ...
            // We generate the random number separately to avoid timing potential
            // cache misses on the client side.
            for (int wi = 0; wi < numWrites; wi++) {
                makeKey(downCast<int>(generateRandom() % numKeys), keyLength, key[wi]);
                Util::genRandomString(value[wi], objectSize);
            }
            // Do the benchmark
            uint64_t start = Cycles::rdtsc();
            for (int wi = 0; wi < numWrites-1; wi++) {
                cluster->write(tid, key[wi], keyLength, value[wi],
                        objectSize, NULL, NULL, asyncReplication);
            }
#if 0
            cluster->write(tid, key[numWrites-1], keyLength, value[numWrites-1],
                    objectSize, NULL, NULL, asyncReplication);
#endif
#if 1
            // Send last write as synchronous write.
            WriteRpc lastWriteRpc(cluster, tid, key[numWrites-1], keyLength,
                    value[numWrites-1], objectSize, NULL, false);
            if (asyncReplication) {
                cluster->sync(&lastWriteRpc);
            }
            lastWriteRpc.wait();
            // Extra sync just in case last synchronous write RPC couldn't
            // sync everything up.
            if (asyncReplication) {
                cluster->sync();
            }
            ...
            if (now >= stop) {
                count = i+1;
                LOG(ERROR, "Time exceeded: stopping test after %d measurements",
                        count);
                break;
            }
        }
        TimeDist result;
        getDist(ticks, &result);
        printf("%12u %14.1f %14.1f %14.1f %14.1f %14.1f\n", numWrites,
                1e06*result.p50, 1e06*result.p90, 1e06*result.p99,
                1e06*result.p999, 1e06*result.p9999);
    }

    // Shutdown slaves.
    sendCommand("done", "done", 1, numClients-1);

    LOG(NOTICE, "Witness reject rate: %2.5f%%",
            static_cast<double>(UnsyncedObjectRpcWrapper::rejectCount) /
            static_cast<double>(UnsyncedObjectRpcWrapper::totalCount) * 100);
    LOG(NOTICE, "Cycles per sec: %f", Cycles::perSecond());
...
//    txSpan
    ...
    // The following variable is used to stop the test after 10 seconds
    // if we haven't read count keys by then.
    uint64_t stop = Cycles::rdtsc() + Cycles::fromSeconds(60.0);
...
writeThroughputMaster(int numObjects, int size, uint16_t keyLength)
{
    ...
    printf("# clients   throughput   worker   cleaner  compactor  "
            "cleaner  dispatch  netOut    netIn   replic    sync    witness\n");
    printf("#           (kops/sec)   cores     cores    free %%    "
            "free %%   utiliz.   (MB/s)    (MB/s)   eff.     frac    GC frac\n");
    printf("#------------------------------------------------------"
            "------------------------------------------------------\n");
    fillTable(dataTable, numObjects, keyLength, size);
...
//        sendCommand("run", "running", numSlaves, 1);
    for (int numSlaves = numClients - 5; numSlaves < numClients; numSlaves++) {
        ...
        cluster->objectServerControl(controlTable, "abc", 3,
                WireFormat::ControlOp::GET_PERF_STATS, NULL, 0,
                &statsBuffer);
        PerfStats witnessStartStats = *statsBuffer.getStart<PerfStats>();
        ...
        cluster->objectServerControl(controlTable, "abc", 3,
                WireFormat::ControlOp::GET_PERF_STATS, NULL, 0,
                &statsBuffer);
        PerfStats witnessFinishStats = *statsBuffer.getStart<PerfStats>();
        ...
        double witnessGcFraction = static_cast<double>(finishStats.temp3 -
                startStats.temp3) / elapsedCycles;
        double witnessRate = static_cast<double>(
                witnessFinishStats.temp1 + witnessFinishStats.temp2 -
                witnessStartStats.temp1 - witnessStartStats.temp2) /
                elapsedTime;
        double witnessAcceptRate = static_cast<double>(
                witnessFinishStats.temp1 - witnessStartStats.temp1) /
                static_cast<double>(
                witnessFinishStats.temp1 + witnessFinishStats.temp2 -
                witnessStartStats.temp1 - witnessStartStats.temp2);
        printf("%5d       %8.2f   %8.3f %8.3f %8.1f  %8.1f  %8.3f "
                "%8.2f  %8.2f %7.2f  %7.2f  %7.2f | %8.2f   %3.3f\n",
                ...
                witnessGcFraction, witnessRate/1e03, witnessAcceptRate*100);
    }
    // Hack to get clean timetrace with full of clients.
            ...
            if (strcmp(command, "run") == 0) {
                ...
                do {
                    ...
                    makeKey(downCast<int>(generateRandom() % numObjects),
                            keyLength, key);
                    Util::genRandomString(value, objectSize);
                    TimeTrace::record("sending write RPC.");
                    cluster->write(dataTable, key, keyLength, value, objectSize,
                            NULL, NULL, asyncReplication);
                    ++objectsWritten;
                } while (Cycles::rdtsc() < checkTime);
 ...
 */
void
writeClusterThroughputMaster(int numObjects, int size, uint16_t keyLength)
{
    // This is the master client. Fill in the table, then measure
    // throughput while gradually increasing the number of workers.
    printf("#\n");
    printf("# clients   throughput   worker   cleaner  throughput  "
            "cleaner  dispatch  netOut    netIn   replic    sync    witness |"
            "  witness    witness  witness  witness  witness witness\n");
    printf("#           (kops/sec)   cores     cores    perServer  "
            "free %%   utiliz.   (MB/s)    (MB/s)   eff.     frac    GC frac |"
            " throughput  worker   cleaner  dispatch netOut  netIn\n");
    printf("#------------------------------------------------------"
            "------------------------------------------------------\n");
...
//        sendCommand("run", "running", numSlaves, 1);
    for (int numSlaves = numClients - (5 * 4 * 4); numSlaves < numClients; numSlaves += 5 * 4) {
        ...
        cluster->serverControlAll(WireFormat::ControlOp::GET_PERF_STATS,
                NULL, 0, &afterStats);
        PerfStats::Int64Diff diff;
        std::vector<double> cyclesPerSecond;
...
//                di < cyclesPerSecond.size(); di++) {
        ...
        uint64_t clusterElapsedCycles = 0;
        uint64_t clusterWriteCount = 0;
        uint64_t clusterWorkerActiveCycles = 0;
        uint64_t clusterCleanerActiveCycles = 0;
        uint64_t clusterDispatchActiveCycles = 0;
        uint64_t clusterNetOutBytes = 0;
        uint64_t clusterNetInBytes = 0;
        uint64_t clusterReplicationRpcs = 0;
        uint64_t clusterLogSyncCycles = 0;
...
//                di < diff["collectionTime"].size(); di++) {
        for (uint di = 0; di < cyclesPerSecond.size(); di++) {
            ...
            clusterCleanerActiveCycles += diff["compactorActiveCycles"][di] +
                                          diff["cleanerActiveCycles"][di];
            clusterDispatchActiveCycles += diff["dispatchActiveCycles"][di];
            clusterNetOutBytes += diff["networkOutputBytes"][di];
            clusterNetInBytes += diff["networkInputBytes"][di];
            clusterReplicationRpcs += diff["replicationRpcs"][di];
            clusterLogSyncCycles += diff["logSyncCycles"][di];
            clusterWitnessGcCycles += diff["temp3"][di];
        }

        uint64_t wclusterElapsedCycles = 0;
        uint64_t wclusterRecordCount = 0;
        uint64_t wclusterWorkerActiveCycles = 0;
        uint64_t wclusterCleanerActiveCycles = 0;
        uint64_t wclusterDispatchActiveCycles = 0;
        uint64_t wclusterNetOutBytes = 0;
        uint64_t wclusterNetInBytes = 0;
...
//                di < cyclesPerSecond.size(); di++) {
        for (uint di = 0; di < cyclesPerSecond.size(); di++) {
            ...
            wclusterCleanerActiveCycles += diff["compactorActiveCycles"][di] +
                                          diff["cleanerActiveCycles"][di];
            wclusterDispatchActiveCycles += diff["dispatchActiveCycles"][di];
            wclusterNetOutBytes += diff["networkOutputBytes"][di];
            wclusterNetInBytes += diff["networkInputBytes"][di];
            wclusterReplicationRpcs += diff["replicationRpcs"][di];
            wclusterLogSyncCycles += diff["logSyncCycles"][di];
            wclusterWitnessGcCycles += diff["temp3"][di];
        }

//        int serverCount = static_cast<int>(cyclesPerSecond.size());
        double elapsedCycles = static_cast<double>(clusterElapsedCycles);
        double elapsedTime = elapsedCycles/ clusterCyclesPerSecond;
        double rate = static_cast<double>(clusterWriteCount) / elapsedTime;
        double utilization = static_cast<double>(clusterWorkerActiveCycles) /
                elapsedCycles;
        double cleanerUtilization = static_cast<double>(
                clusterCleanerActiveCycles) / elapsedCycles;
        double dispatchUtilization = static_cast<double>(
                clusterDispatchActiveCycles) / elapsedCycles;
        double netOutRate = static_cast<double>(
                clusterNetOutBytes) / elapsedTime;
        double netInRate = static_cast<double>(
                clusterNetInBytes) / elapsedTime;
        double repEfficiency = static_cast<double>(clusterWriteCount) /
                static_cast<double>(clusterReplicationRpcs);
        double syncFraction = static_cast<double>(clusterLogSyncCycles) /
                elapsedCycles;
        double witnessGcFraction = static_cast<double>(clusterWitnessGcCycles) /
                elapsedCycles;

        double welapsedCycles = static_cast<double>(wclusterElapsedCycles);
        double wrate = static_cast<double>(wclusterRecordCount) / elapsedTime;
        double wutilization = static_cast<double>(wclusterWorkerActiveCycles) /
                welapsedCycles;
        double wcleanerUtilization = static_cast<double>(
                wclusterCleanerActiveCycles) / welapsedCycles;
        double wdispatchUtilization = static_cast<double>(
                wclusterDispatchActiveCycles) / welapsedCycles;
        double wnetOutRate = static_cast<double>(
                wclusterNetOutBytes) / elapsedTime;
        double wnetInRate = static_cast<double>(
                wclusterNetInBytes) / elapsedTime;
        ...
        printf("%5d       %8.2f   %8.3f %8.3f %8.1f  %8.1f  %8.3f "
                ...
                numSlaves, rate/1e03, utilization, cleanerUtilization,
                rate/1e03/numMasters, 0.0, dispatchUtilization,
                netOutRate/1e06/numMasters, netInRate/1e06/numMasters, repEfficiency, syncFraction,
                witnessGcFraction, wrate/1e03/numWitnesses, wutilization,
                wcleanerUtilization, wdispatchUtilization,
                wnetOutRate/numWitnesses/1e06, wnetInRate/numWitnesses/1e06);
    }
    // Hack to get clean timetrace with full of clients.
    cluster->serverControlAll(WireFormat::LOG_TIME_TRACE);
    Cycles::sleep(1000000);
...
writeClusterThroughput()
{
    ...
    if (clientIndex == 0) {
        // This is the master client.
        printf("# RAMCloud write throughput of all servers (%d masters) in"
                " cluster with a varying\n"
                "# number of clients issuing individual write on randomly\n"
                "# chosen %d-byte objects with %d-byte keys\n",
                numMasters, size, keyLength);
        printf("# Generated by 'clusterperf.py writeThroughput'\n");
        writeClusterThroughputMaster(numObjects, size, keyLength);
    } else {
        // Slaves execute the following code, which creates load by
        // issuing individual reads.
        bool running = false;
        ...
        while (true) {
            ...
            if (strcmp(command, "run") == 0) {
                if (!running) {
                    ...
                    running = true;
                    char tableName[20];
                    snprintf(tableName, sizeof(tableName), "dataNum%d",
                            clientIndex % numMasters);
                    tid = cluster->getTableId(tableName);
                    RAMCLOUD_LOG(NOTICE,
                            "Starting writeThroughput benchmark");
                }
...
TestInfo tests[] = {
    ...
    {"readThroughputWhileBeingWitness", readThroughputWhileBeingWitness},
    ...
    {"writeClusterThroughput", writeClusterThroughput},
    {"workloadThroughput", workloadThroughput},
    {"writeRandomMultipleAndSync", writeRandomMultipleAndSync},
};
...
try
{
    ...
    clusterperfOptions.add_options()
        ...
        ("numMasters", po::value<int>(&numMasters)->default_value(1),
                "Total number of master servers running")
        ("numWitnesses", po::value<int>(&numWitnesses)->default_value(1),
                "Total number of witness servers running")
