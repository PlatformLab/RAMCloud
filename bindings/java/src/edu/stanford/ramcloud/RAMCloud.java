...
public class RAMCloud {
     ...
     *
     * @param rules RejectRules object to convert to a byte array.
     ...
     *
     * @param locator Describes how to locate the coordinator. It can have
     * either of two forms. The preferred form is a locator for external storage
     * that contains the cluster configuration information (such as a string
     * starting with "zk:", which will be passed to the ZooStorage constructor).
     * With this form, sessions can automatically be redirected to a new
     * coordinator if the current one crashes. Typically the value for this
     * argument will be the same as the value of the "-x" command-line option
     * given to the coordinator when it started. The second form is deprecated,
     * but is retained for testing. In this form, the location is specified as a
     * RAMCloud service locator for a specific coordinator. With this form it is
     * not possible to roll over to a different coordinator if a given one
     * fails; we will have to wait for the specified coordinator to restart.
     * @param clusterName Name of the current cluster. Used to allow independent
     * operation of several clusters sharing many of the same resources. This is
     ...
     * Read the current contents of an object.
     *
     * @param tableId The table containing the desired object (return value from
     * a previous call to getTableId).
     * @param key Variable length key that uniquely identifies the object within
     * tableId. It does not necessarily have to be null terminated. The caller
     * must ensure that the storage for this key is unchanged through the life
     * of the RPC.
     * @param rules If non-NULL, specifies conditions under which the read
    ...
    public RAMCloudObject read(long tableId, byte[] key, RejectRules rules) {
        ...
        int[] status = new int[1];
        byte[] value = RAMCloud.cppRead(ramcloudObjectPointer, tableId, key, ruleBytes, version, status);
     ...
     *
     * @param tableId The table containing the object to be deleted (return
     * value from a previous call to getTableId).
     * @param key Variable length key that uniquely identifies the object within
     * tableId.
     * @param rules If non-NULL, specifies conditions under which the delete
     * should be aborted with an error.
    ...
    public long remove(long tableId, byte[] key, RejectRules rules) {
        byte[] ruleBytes = RAMCloud.getRejectRulesBytes(rules);
        int[] status = new int[1];
        long out = RAMCloud.cppRemove(ramcloudObjectPointer, tableId, key, ruleBytes, status);
        checkStatus(status);
        return out;
    }
     ...
     * @param key Variable length key that uniquely identifies the object within
     * tableId.
     * @param value String providing the new value for the object.
     * @param rules If non-NULL, specifies conditions under which the write
     * should be aborted with an error.
     * @return The version number of the object is returned. If the operation
     * was successful this will be the new version for the object. If the
     * operation failed then the version number returned is the current version
     * of the object, or 0 if the object does not exist.
     */
    public long write(long tableId, byte[] key, byte[] value, RejectRules rules) {
        byte[] ruleBytes = RAMCloud.getRejectRulesBytes(rules);
        int[] status = new int[1];
        long out = RAMCloud.cppWrite(ramcloudObjectPointer, tableId, key, value, ruleBytes, status);
        checkStatus(status);
        return out;
    }
     ...
     * @param serverSpan The number of servers across which this table will be
     * divided (defaults to 1). Keys within the table will be evenly distributed
     * to this number of servers according to their hash. This is a temporary
     * work-around until tablet migration is complete; until then, we must place
     * tablets on servers statically.
     * @return The return value is an identifier for the created table; this is
     * used instead of the table's name for most RAMCloud operations involving
     * the table.
     */
    public long createTable(String name, int serverSpan) {
        int[] status = new int[1];
        long out = RAMCloud.cppCreateTable(ramcloudObjectPointer, name, serverSpan, status);
        checkStatus(status);
        return out;
    }
     ...
     * instead of the table's name for most RAMCloud operations involving the
     * table.
     */
    ...
    private static native long cppConnect(String locator,
            String clusterName,
            int[] status);
    ...
    private static native long cppRemove(long ramcloudObjectPointer, long tableId,
            byte[] key, byte[] rules,
            int[] status);

    private static native long cppWrite(long ramcloudObjectPointer, long tableId,
            byte[] key, byte[] value,
            byte[] rules,
            int[] status);
}
