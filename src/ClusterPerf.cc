...
transactionDistRandom()
{
    ...
    uint64_t* tableIds = createTables(numTables, 0, "0", 1);

    for (int i = 0; i < numTables; i++) {
        fillTable(tableIds[i], numKeys, keyLength, objectSize);
    }
    ...
    for (int i = 0; i < count; i++) {
        ...
        for (auto it = keyIds.begin(); it != keyIds.end(); it++) {
            ...
            Buffer buffer;
            t.read(tableIds[j], key, keyLength, &buffer);
            t.write(tableIds[j], key, keyLength, value, objectSize);
            j = (j + 1) % numTables;
        }
