...
void
renewLeaseThroughput()
{
    WireFormat::ClientLease leases[1000];

    if (clientIndex == 0) {
        printf("# RAMCloud renewLease throughput with a varying\n"
                "# number of clients\n");
        printf("# Generated by 'clusterperf.py renewLeaseThroughput'\n");
        printf("#\n");
        printf("# numClients   throughput\n");
        printf("#              (kRequests/sec)\n");
        printf("#-------------------------\n");
        for (int numSlaves = 1; numSlaves < numClients; numSlaves++) {
            sendCommand("ready", "done", 1, numSlaves);
            sendCommand("start", "running", 1, numSlaves);
    ...
    for (int i = 0; i < 1000; i++) {
        leases[i] = CoordinatorClient::renewLease(cluster->clientContext, 0);
    }
    ...
    while (true) {
        ...
        if (strcmp(command, "stop") == 0) {
            ...
            if (running && elapsed >= runCycles) {
                double throughput = opCount/Cycles::toSeconds(elapsed);
                sendMetrics(throughput);
                running = false;
                setSlaveState("done");
                RAMCLOUD_LOG(DEBUG, "Run Complete: %f", throughput);
        ...
        // Do benchmark
        WireFormat::ClientLease newLease =
                CoordinatorClient::renewLease(
                        cluster->clientContext, leases[i].leaseId);
        elapsed = Cycles::rdtsc() - startCycles;
        if (newLease.leaseId != leases[i].leaseId) {
            RAMCLOUD_CLOG(WARNING, "Unexpected lease expiration");
            leases[i] = newLease;
        }
        i = (i + 1) % 1000;
...
TestInfo tests[] = {
    ...
    {"renewLeaseThroughput", renewLeaseThroughput},
};
