...
timeIndexWrite(uint64_t tableId, uint8_t numKeys, KeyInfo *keyList,
                                const void* buf, uint32_t length, double ms,
                                double& outTimeWrite, double& outTimeOverwrite)
{
    ...
    //        start = Cycles::rdtsc();    
    //        cluster->write(tableId, numKeys, keyList, buf, length);
    //        timeWrite += Cycles::rdtsc() - start;

    //        start = Cycles::rdtsc();   
    //        cluster->write(tableId, numKeys, keyList, buf, length);
    //        timeOverwrite += Cycles::rdtsc() - start;
    ...
    outTimeWrite =  Cycles::toSeconds(timeWrite/count);
    outTimeOverwrite = Cycles::toSeconds(timeOverwrite/count);
}


double
timeLookupIndexKeys(uint64_t tableId, uint8_t indexId,
            const void* firstKey, uint16_t firstKeyLength,
            uint64_t firstAllowedKeyHash,
            const void* lastKey, uint16_t lastKeyLength,
            Buffer* responseBuffer,
            uint32_t* numHashes, uint16_t* nextKeyLength,
            uint64_t* nextKeyHash, double ms)
{
...
double
timeIndexedRead(uint64_t tableId, uint32_t numHashes,
            Buffer* pKHashes, uint8_t indexId,
            const void* firstKey, uint16_t firstKeyLength,
            const void* lastKey, uint16_t lastKeyLength,
            Buffer* response, uint32_t* numObjects, double ms)
{
    uint64_t runCycles = Cycles::fromSeconds(ms/1e03);

    //response->reset();
    cluster->indexedRead(tableId, numHashes, pKHashes, indexId, firstKey,
                        firstKeyLength, lastKey, lastKeyLength, response,
                        numObjects);
...
indexExp1()
{
    ...
    std::vector<double> writeL, lookupL, readL;
    writeL.resize(numObjects);
    lookupL.resize(numObjects);
    readL.resize(numObjects);
    ...
    for (int i=0; i<numObjects; i++){
        ...
        lookupL[i] = timeLookupIndexKeys(dataTable, indexId, keyList[1].key,
        ...
        if (numHashes!=1)
            printf("failed object id:%d, secKey:%s numHashes:%d\n", i, secondaryKey, numHashes);
        assert(numHashes>0);
        ...
        // Buffer readResp;
        // uint32_t numObjects;
        // readL[i] = timeIndexedRead(dataTable, numHashes, &pKHashes, indexId,
        //             keyList[1].key, keyList[1].keyLength, keyList[1].key,
        //             keyList[1].keyLength, &readResp, &numObjects, ms);

        if(i%100==0){
                printf("%d) ", i);
                printTime("write", writeL[i], "");
                printTime("overwrite", timeOverwrite, "");
                printTime("lookup", lookupL[i], "");
                //printTime("read", readL[i], "");
                printf("\n");
        }
    }
...
indexExp2()
{
    ...
    int numObjects = 100;
    uint8_t maxNumKeys = 20;
    int constSize = (maxNumKeys+2)*30;
    double ms = 100;

    std::vector<double> writeL, overwriteL, removeL;
    writeL.resize(maxNumKeys);
    overwriteL.resize(maxNumKeys);
    removeL.resize(maxNumKeys);

    for (uint8_t numKeys=1; numKeys<maxNumKeys; numKeys++){
        ...
        for (int i=0; i<numObjects; i++){
            ...
            char key[numKeys+1][30];
            for (int j=0; j<numKeys+1; j++){
                snprintf(key[j], sizeof(key[j]), "%d|%d|%0*d", i, j, 30, 0);
                keyList[j].keyLength = 30;
                keyList[j].key = key[j];
            }
            
            Buffer input;
            int size = constSize - 30*(numKeys+1);
            fillBuffer(input, size, indexTable,
                       keyList[0].key, keyList[0].keyLength);
            
            if(i==numObjects-1){
                timeIndexWrite(indexTable, (uint8_t)(numKeys+1), keyList,
                    input.getRange(0, size), size, ms, writeL[numKeys], 
                    overwriteL[numKeys]);
            }
            else
            ...
            //verify
            if(i!=numObjects-1){
                Key secKey(indexTable, keyList[0].key, keyList[0].keyLength);
                for (uint8_t z=1; z<=numKeys; z++){    
                    Buffer lookupResp;
                    uint32_t numHashes;
                    uint16_t nextKeyLength;
                    ...
                    printf("lookup index id: %d object:%d\n", z, i);
                    assert(1==numHashes);
                    lookupOffset = sizeof32(WireFormat::LookupIndexKeys::Response);
                    assert(secKey.getHash()==
                                    *lookupResp.getOffset<uint64_t>(lookupOffset));
                }
            }
        ...
        printf("%d) ", numKeys);
        printTime("write", writeL[numKeys], "");
        printTime("overwrite", overwriteL[numKeys], "");
        printf("\n");
    }
}
...
{   
        ...
        while (true) {
            ...
            if (strcmp(command, "run") == 0) {
                ...
                while (true) {
                    ...
                    uint16_t nextKeyLength;
                    uint64_t nextKeyHash;
                    cluster->lookupIndexKeys(dataTable, indexId, "a",
                        1, firstAllowedKeyHash, "b", 1, &lookupResp,
                        &numHashes, &nextKeyLength, &nextKeyHash);

                    int currentSize = numHashes;
                    if (currentSize != 0) {
                        if (start == 0) {
                            start = Cycles::rdtsc();
                            size = currentSize;
    ...
    //measure thorughput from master
    printf("# Generated by 'clusterperf.py indexExp3'\n");
    printf("#\n");
    printf("# numClients  lookupLatency(us)  throughput(total klookups/sec)\n");
    printf("#----------------------------------------------------------\n");
    for (int numSlaves = 0; numSlaves < numClients; numSlaves++) {
        ...
        uint16_t nextKeyLength;
        uint64_t nextKeyHash;
        double t = timeLookupIndexKeys(dataTable, indexId, "a",
            1, firstAllowedKeyHash, "b", 1, &lookupResp,
            &numHashes, &nextKeyLength, &nextKeyHash, ms);
