...
class AlarmSession : public Transport::Session {
  public:
    ...
    void finishRpcs(const char *message)
    {
        while (rpcs.size() > 0) {
    ...
    abort(const char *message)
    {
        ...
        log.append(format("abort: %s", message));
        finishRpcs(message);
    }
...
TEST_F(SessionAlarmTest, basics) {
    ...
    for (int i = 0; i < 10; i++) {
        ...
        while (AlarmSession::log.find("server is not responding")
        ...
        EXPECT_EQ(format("clientSend: opcode %d, clientSend: opcode %d, "
                "abort: server is not responding", RpcOpcode::READ,
                RpcOpcode::PING), AlarmSession::log);
...
TEST_F(SessionAlarmTimerTest, handleTimerEvent_pingAndabortSession) {
    ...
    AlarmSession::log.clear();
    timer.handleTimerEvent();
    EXPECT_EQ("abort: server is not responding", AlarmSession::log);
}
