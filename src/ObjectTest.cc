...
/**
 * Unit tests for RpcRecord.
 */
class RpcRecordTest : public ::testing::Test {
  public:
    RpcRecordTest()
        : stringKey(),
          ...
          rpcRecordFromBuffer()
    {
        ...
        rpcRecordFromResponse.construct(key.getTableId(),
                                        key.getHash(),
                                        1UL,
                                        10UL,
                                        9UL,
        ...
        rpcRecordFromResponse->assembleForLog(buffer2);
        ...
        rpcRecordFromBuffer.construct(buffer2, sizeof32(stringKey),
                                   buffer2.size() - sizeof32(stringKey));

        records[0] = &*rpcRecordFromResponse;
        records[1] = &*rpcRecordFromBuffer;
    }
    ...
    Tub<RpcRecord> rpcRecordFromResponse;
    Tub<RpcRecord> rpcRecordFromBuffer;
    //TODO(seojin): Test for constructor with response with contiguous memory.

    RpcRecord* records[2];

    DISALLOW_COPY_AND_ASSIGN(RpcRecordTest);
};

TEST_F(RpcRecordTest, constructor_fromResponse) {
    RpcRecord& record = *records[0];
    Key key(572, stringKey, 5);
...
TEST_F(RpcRecordTest, constructor_fromBuffer) {
    RpcRecord& record = *records[1];
    Key key(572, stringKey, 5);
    ...
    EXPECT_EQ(5 + sizeof(RpcRecord::Header)
    ...
    WireFormat::Write::Response* resp =
        reinterpret_cast<WireFormat::Write::Response*>(
            record.respBuffer->getRange(sizeof(RpcRecord::Header) + 5,
                 sizeof(WireFormat::Write::Response)));
    EXPECT_EQ(Status::STATUS_OK, resp->common.status);
    EXPECT_EQ(123UL, resp->version);
}

TEST_F(RpcRecordTest, assembleForLog) {
    Key key(572, stringKey, 5);
    for (uint32_t i = 0; i < arrayLength(records); i++) {
        RpcRecord& record = *records[i];
        Buffer buffer;
        record.assembleForLog(buffer);
        const RpcRecord::Header* header =
            buffer.getStart<RpcRecord::Header>();
...
TEST_F(RpcRecordTest, assembleForLog_contigMemory) {
    Key key(572, stringKey, 5);
    for (uint32_t i = 0; i < arrayLength(records); i++) {
        RpcRecord& record = *records[i];
        Buffer buffer;
        ...
        RpcRecord::Header* header =
            reinterpret_cast<RpcRecord::Header*>(target);
...
TEST_F(RpcRecordTest, appendRespToBuffer) {
    for (uint32_t i = 0; i < arrayLength(records); i++) {
        RpcRecord& record = *records[i];
        Buffer buffer;
...
TEST_F(RpcRecordTest, getTableId) {
    for (uint32_t i = 0; i < arrayLength(records); i++)
        EXPECT_EQ(572U, records[i]->getTableId());
}

TEST_F(RpcRecordTest, getKeyHash) {
    Key key(572, stringKey, 5);
    for (uint32_t i = 0; i < arrayLength(records); i++)
        EXPECT_EQ(key.getHash(), records[i]->getKeyHash());
}

TEST_F(RpcRecordTest, getLeaseId) {
    for (uint32_t i = 0; i < arrayLength(records); i++)
        EXPECT_EQ(1U, records[i]->getLeaseId());
}

TEST_F(RpcRecordTest, getRpcId) {
    for (uint32_t i = 0; i < arrayLength(records); i++)
        EXPECT_EQ(10U, records[i]->getRpcId());
}

TEST_F(RpcRecordTest, getAckId) {
    for (uint32_t i = 0; i < arrayLength(records); i++)
        EXPECT_EQ(9U, records[i]->getAckId());
}

TEST_F(RpcRecordTest, getResp) {
    for (uint32_t i = 0; i < arrayLength(records); i++) {
        RpcRecord& record = *records[i];
...
TEST_F(RpcRecordTest, getRespLength) {
    for (uint32_t i = 0; i < arrayLength(records); i++)
        EXPECT_EQ(sizeof(WireFormat::Write::Response),
                  records[i]->getRespLength());
}

TEST_F(RpcRecordTest, checkIntegrity) {
    for (uint32_t i = 0; i < arrayLength(records); i++) {
        RpcRecord& record = *records[i];
        Buffer buffer;
        record.assembleForLog(buffer);
...
TEST_F(RpcRecordTest, getSerializedLength) {
    for (uint32_t i = 0; i < arrayLength(records); i++)
