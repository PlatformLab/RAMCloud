 ...
 * \param respLen
 *      Length of response.
 *      A length of 0 means that the RpcRecord occupies the entire buffer
 *      starting at offset.
 */
 ...
 *
 * \param buffer
 *      Buffer pointing to a complete serialized RpcRecord. It is the
 *      caller's responsibility to make sure that the buffer passed in
 *      actually contains a full RpcRecord. If it does not, then behavior
 *      is undefined.
 * \param offset
 *      Starting offset in the buffer where the RpcRecord begins.
 * \param length
 *      Total length of the RpcRecord in bytes.
 *      A length of 0 means that the RpcRecord occupies the entire buffer
 *      starting at offset.
 */
RpcRecord::RpcRecord(Buffer& buffer, uint32_t offset, uint32_t length)
    : header(*buffer.getOffset<Header>(offset)),
      respLength(),
      response(),
      respBuffer(&buffer),
...
/**
 * Append the full RpcRecord, including header, and response exactly as it
 * should be stored in the log, to a buffer
 *
 * \param buffer
 *      The buffer to append a serialized version of this RpcRecord to.
 */
void
RpcRecord::assembleForLog(Buffer& buffer)
{
    header.checksum = computeChecksum();
...
/**
 * Copy the full serialized RpcRecord including the header and the response
 * to a contiguous region of memory memBlock.
 *
 * \param memBlock
 *      Destination to copy a serialized version of this RpcRecord to.
 *      The caller must ensure that sufficient memory is allocated for the
 *      complete RpcRecord to be stored.
 */
void
RpcRecord::assembleForLog(void* memBlock)
{
    uint8_t *dst = reinterpret_cast<uint8_t*>(memBlock);
    header.checksum = computeChecksum();
...
/**
 * Append the response portion of this RpcRecord to a provided
 * buffer.
 *
 * \param buffer
 ...
 */
void
RpcRecord::appendRespToBuffer(Buffer& buffer)
{
    if (response) {
 ...
 *
 * \return
 *      NULL if the RpcRecord is malformed,
 *      a pointer to a contiguous copy of the RPC's response otherwise
 */
const void*
RpcRecord::getResp(uint32_t *length)
{
    if (length)
        *length = respLength;
...
uint32_t
RpcRecord::getRespLength()
{
    return respLength;
}

/**
 * Compute a checksum on the RpcRecord and determine whether or not it matches
 * what is stored in the RpcRecord. Returns true if the checksum looks ok,
 * otherwise returns false.
 */
bool
RpcRecord::checkIntegrity()
{
    return computeChecksum() == header.checksum;
}

/**
 * Obtain the total size of the RpcRecord including its header.
 */
uint32_t
RpcRecord::getSerializedLength()
{
    return sizeof32(header) + respLength;
}

/**
 * Compute the RpcRecord's checksum and return it.
 */
uint32_t
RpcRecord::computeChecksum()
{
    assert(OFFSET_OF(Header, checksum) ==
        (sizeof(header) - sizeof(header.checksum)));
