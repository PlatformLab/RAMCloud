...
basic()
{
    ...
    for (int i = 0; i < 5; i++) {
        ...
        snprintf(name, 50, "basic::read%s", ids[i]);
        snprintf(description, 50, "read single %sB object", ids[i]);
        printTime(name, t, description);
        snprintf(name, 50, "basic::readBw%s", ids[i]);
        snprintf(description, 50, "bandwidth reading %sB object", ids[i]);
    ...
    for (int i = 0; i < 5; i++) {
        ...
        snprintf(name, 50, "basic::write%s", ids[i]);
        snprintf(description, 50, "write single %sB object", ids[i]);
        printTime(name, t, description);
        snprintf(name, 50, "basic::writeBw%s", ids[i]);
        snprintf(description, 50, "bandwidth writing %sB object", ids[i]);
        printBandwidth(name, size/t, description);
    }
}
