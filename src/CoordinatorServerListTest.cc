...
TEST_F(CoordinatorServerListTest, hasUpdates) {
    ...
    sl.serverList[id1.indexNumber()].entry->isBeingUpdated = true;
    EXPECT_FALSE(sl.hasUpdates(lock));
...
TEST_F(CoordinatorServerListTest, hasUpdates_partialPrune) {
    ...
    sl.serverList[id1.indexNumber()].entry->isBeingUpdated = true;
    sl.serverList[id2.indexNumber()].entry->isBeingUpdated = true;
    sl.serverList[id3.indexNumber()].entry->isBeingUpdated = true;
    EXPECT_EQ(3UL, sl.updates.size());
...
TEST_F(CoordinatorServerListTest, updateEntryVersion) {
    ...
    EXPECT_FALSE(sl.getReferenceFromServerId(id1).isBeingUpdated);
}
...
TEST_F(CoordinatorServerListTest, updateLoop) {
    ...
    EXPECT_EQ("sendRequest: 0x40023 11 529 0 /0 /x18/0 | "
        "sendRequest: 0x40023 146 0x100d2b0a 0x11000000 1 1 0x6f6d111a "
        "ck:host=server5-/0 0x35000000 0 0x100d2b0a 0x11000000 5 0 0x6f6d111a "
        "ck:host=server6-/0 0x35000000 0 0x100d2b0a 0x11000000 5 0 0x6f6d111a "
        "ck:host=server6-/0 0x35000000 1 529 0 /0 /x18/x01",
        transport.outputLog);
}
