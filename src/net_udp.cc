...
int
rc_net_send(struct rc_net *net, void *buf/*void *buf, size_t len*/)
{
    if (!net->connected)
        assert(!rc_net_connect(net));

    RAMCloud::BufferPtr *buf_ptr = (RAMCloud::BufferPtr *) buf;

    uint8_t* raw_buf = (uint8_t *) malloc(sizeof(uint8_t) * buf_ptr->totalLength());
    buf_ptr->copy(0, buf_ptr->totalLength(), raw_buf);

    if (sendto(net->fd, raw_buf, buf_ptr->totalLength(), 0,
...
int
rc_net_send_rpc(struct rc_net *net, void* buf/*struct rcrpc_any *rpc*/)
{
    return rc_net_send(net, buf/*rpc, rpc->header.len*/);
}


int
rc_net_recv(struct rc_net *net, void **buf/*void **buf, size_t *buflen*/)
{
    if (!net->connected)
        assert(!rc_net_connect(net));
...
int
rc_net_recv_rpc(struct rc_net *net, void **buf/*struct rcrpc_any **rpc*/)
{
    size_t len;
    return rc_net_recv(net, buf/*, &len*/);
}
