 ...
 * object.
 */
Syscall* TcpTransport2::sys = &defaultSyscall;
 ...
 */
TcpTransport2::TcpTransport2(const ServiceLocator* serviceLocator)
        ...
        waitingRequests()
{
    ...
    if (event_add(&listenSocketEvent, NULL) != 0) {
        throw TransportException(HERE,
                "event_add failed in TcpTransport2 constructor");
    }
}

/**
 * Destructor for TcpTransport2's: close file descriptors and perform
 ...
 */
void
TcpTransport2::closeSocket(int fd) {
 ...
 * \param arg
 *      Pointer to the TcpTransport2 object whose listenSocket became
 *      readable.
 */
void
TcpTransport2::tryAccept(int fd, int16_t event, void* arg)
{
    TcpTransport2* transport = static_cast<TcpTransport2*>(arg);
 ...
 *      The event that happened: should always be EV_READ.
 * \param arg
 *      Pointer to TcpTransport2 object associated with the socket.
 */
void
TcpTransport2::tryServerRecv(int fd, int16_t event, void *arg)
{
    TcpTransport2* transport = static_cast<TcpTransport2*>(arg);
    ...
    try {
        if (socket->busy) {
            ...
            char buffer[2000];
            ssize_t length = TcpTransport2::recvCarefully(fd, buffer,
                    sizeof(buffer));
            if (length > 0) {
 ...
 */
void
TcpTransport2::sendMessage(int fd, Buffer& payload)
{
    assert(fd >= 0);
...
ssize_t
TcpTransport2::recvCarefully(int fd, void* buffer, size_t length) {
 ...
 *      the buffer is empty.
 */
TcpTransport2::IncomingMessage::IncomingMessage(Buffer* buffer)
 ...
 *      the buffer is empty.
 */
void TcpTransport2::IncomingMessage::reset(Buffer* buffer)
{
...
TcpTransport2::IncomingMessage::readMessage(int fd) {
    ...
    // calls to this method before we get all of it).
    {
        ...
        ssize_t len = TcpTransport2::recvCarefully(fd, dest,
 ...
 */
TcpTransport2::TcpSession::TcpSession(const ServiceLocator& serviceLocator)
 ...
 */
TcpTransport2::TcpSession::~TcpSession()
{
    close();
}
 ...
 */
void
TcpTransport2::TcpSession::close()
{
    if (fd >= 0) {
        event_del(&readEvent);
        sys->close(fd);
...
TcpTransport2::ClientRpc*
TcpTransport2::TcpSession::clientSend(Buffer* request, Buffer* reply)
{
    while (current != NULL) {
 ...
 */
void
TcpTransport2::TcpSession::tryReadReply(int fd, int16_t event, void *arg)
{
...
void
TcpTransport2::TcpServerRpc::sendReply()
{
    assert(socket->busy);
    ...
    TcpTransport2::sendMessage(fd, replyPayload);
    socket->busy = false;
}
