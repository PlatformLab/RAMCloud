 ...
 *
 * \param ramcloud
 *      Overall information about the client.
 */
ClientTransactionManager::ClientTransactionManager(RamCloud* ramcloud)
 ...
 */
void
ClientTransactionManager::addTransactionTask(
 ...
 */
void
ClientTransactionManager::handleTimerEvent()
{
    ...
    // Reschedule if there are more tasks that need running.
    if (!taskList.empty()) {
        this->start(0);
    }
}
