            ...
            enum RCRPC_TYPE resp_type, size_t min_resp_size)
{
    ...
    *respp = NULL;
    ...
    RAMCloud::BufferPtr *bp = new RAMCloud::BufferPtr();
    bp->append(&req, req->header.len);
    assert(!rc_net_send_rpc(net, (void *) bp));
    delete bp;

    bp = new RAMCloud::BufferPtr();
    assert(!rc_net_recv_rpc(net, (void **) &bp/*resp*/));
...
struct rc_client *
rc_new(void) {
    return (struct rc_client *) malloc(sizeof(struct rc_client));
}
