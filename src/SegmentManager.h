...
class SegmentManager {
  public:
    ...
    bool increaseSurvivorReserve(uint32_t numSegments);
  ...
  PRIVATE:
    ...
    /**
     * The SegmentManager allocates segments for two main purposes: new heads
     * of the log, and survivor segments generated by the cleaner. The system
     * reserves some segments for these different uses, so allocation requests
     * must specify the desired type.
     */
    enum AllocationType {
        /// Allocate a free segment, if available, to serve as a mutable log
        /// head.
        ALLOC_HEAD = 0,

        /// Allocate one of the specially-reserved segments to serve as an
        /// immutable log head when low on memory.
        ALLOC_EMERGENCY_HEAD = 1,

        /// Allocate one of the segments reserved for writing survivor data to
        /// by the log cleaner.
        ALLOC_SURVIVOR = 2
    };
    ...
    bool mayAlloc(AllocationType type);
    LogSegment* alloc(AllocationType type);
    ...
    /// The number of emergency head segments we keep reserved for exigent
    /// circumstances.
    ///
    ...
    ///    log digest and doesn't include them.
    ///
    ...
    /// to keep around two emergency head segments that we can open in such a
    /// situation and immediately reclaim once the next head is opened.
    const size_t numEmergencyHeads;

    /// The number of emergency heads that have currently been allocated. Used
    /// in conjunction with numEmergencyHeads to determine how many segments are
    /// not being reserved for emergency heads.
    size_t numEmergencyHeadsAlloced;
    ...
    /// cleaner. Used in conjunction with numSurvivorSegments to determine how
    /// many segments are not being reserved for the cleaner.
...
class SegmentManager::Allocator : public Segment::Allocator {
  public:
    ...
    getFreeSegmentCount()
    {
        ...
        return getFreeSegmentCount(guard);
    }
    ...
    alloc()
    {
        ...
        if (getFreeSegmentCount(guard) == 0)
  ...
  PRIVATE:
    ...
    /**
     * Unlocked version of getFreeSegmentCount().
     */
    uint32_t
    getFreeSegmentCount(std::lock_guard<SpinLock>& lockGuard)
    {
    ...
    /// Monitor-style lock protecting the freeList. This allocator is mostly
    /// invoked under the SegmentManager's monitor lock, but the cleaner also
    /// requests segments to return unused seglets to the allocator outside of
    /// that lock.
    SpinLock lock;
