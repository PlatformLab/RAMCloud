...
BasicTransport::deleteClientRpc(ClientRpc* clientRpc)
{
    ...
    NL_TRACE("deleting client RPC, sequence %u",
    ...
    NL_TRACE("deleted client RPC, clientId %lu, sequence %lu, "
            "%lu outgoing RPCs",
...
BasicTransport::deleteServerRpc(ServerRpc* serverRpc)
{
    ...
    NL_TRACE("deleted server RPC, clientId %lu, sequence %lu, "
            "%lu incoming RPCs",
    ...
    while ((curOffset < messageSize) && (bytesSent < maxBytes)) {
        ...
        if (bytesThisPacket == messageSize) {
            ...
            if (flags & FROM_CLIENT) {
                NL_TRACE("client sending ALL_DATA, clientId %lu, sequence %lu",
                         rpcId.clientId, rpcId.sequence);
            } else {
                NL_TRACE("server sending ALL_DATA, clientId %lu, sequence %lu",
                         rpcId.clientId, rpcId.sequence);
            }
            ...
            if (flags & FROM_CLIENT) {
                NL_TRACE("client sending DATA, clientId %lu, sequence %lu, "
                         "offset %u",
                        rpcId.clientId, rpcId.sequence, curOffset);
            } else {
                NL_TRACE("server sending DATA, clientId %lu, sequence %lu, "
                         "offset %u",
                        rpcId.clientId, rpcId.sequence, curOffset);
            }
            driver->sendPacket(address, &header, &iter, 0, &txQueueState);
        ...
        if (txQueueState.outstandingBytes > 0) {
            NL_TRACE("sent data, %u bytes queued ahead",
                    txQueueState.outstandingBytes);
        } else {
            NL_TRACE("sent data, tx queue idle time %lu cyc",
        ...
        const T* packet)
{
    ...
    NL_TRACE("sent control packet, opcode %u, %u bytes queued ahead, "
            "tx queue idle time %lu cyc", packet->common.opcode,
...
BasicTransport::tryToTransmitData()
{
    ...
    while (transmitQueueSpace > 0) {
        ...
        if (expect_false((NULL == message) && transmitDataSlowPath)) {
            NL_TRACE("slow path taken, iterating over %lu outgoing messages",
                ...
                RpcNotifier* notifier)
{
    ...
    NL_TRACE("sendRequest invoked, clientId %lu, sequence %lu, length %u, "
            "%lu outgoing requests", t->clientId, t->nextClientSequenceNumber,
    ...
    if (bytesSent > 0) {
        NL_TRACE("sendRequest transmitted %u bytes", bytesSent);
    }
}
    ...
    if (!(common->flags & FROM_CLIENT)) {
        ...
        switch (common->opcode) {
            ...
            case PacketOpcode::ALL_DATA: {
                ...
                NL_TRACE("client received ALL_DATA, clientId %lu, "
            ...
            case PacketOpcode::DATA: {
                ...
                if (header == NULL)
                    goto packetLengthError;
                NL_TRACE("client received DATA, clientId %lu, sequence %lu, "
                        "offset %u, length %u",
                        header->common.rpcId.clientId,
            ...
            case PacketOpcode::GRANT: {
                ...
                if (header == NULL)
                    goto packetLengthError;
                NL_TRACE("client received GRANT, clientId %lu, sequence %lu, "
                        "offset %u",
                        header->common.rpcId.clientId,
                        header->common.rpcId.sequence,
                        header->offset);
            ...
            case PacketOpcode::LOG_TIME_TRACE: {
                ...
                TimeTrace::record("client received LOG_TIME_TRACE for "
                        ...
                        (uint32_t)common->rpcId.clientId,
                        (uint32_t)common->rpcId.sequence);
                NL_TRACE("client received LOG_TIME_TRACE for "
                        "clientId %u, sequence %u",
                        (uint32_t)common->rpcId.clientId,
                        (uint32_t)common->rpcId.sequence);
                TimeTrace::printToLogBackground(context->dispatch);
            ...
            case PacketOpcode::RESEND: {
                ...
                if (header == NULL)
                    goto packetLengthError;
                NL_TRACE("client received RESEND, clientId %lu, sequence %lu, "
                        "offset %u, length %u",
                        header->common.rpcId.clientId,
                        header->common.rpcId.sequence,
            ...
            case PacketOpcode::ACK: {
                // Nothing to do.
                NL_TRACE("client received ACK, clientId %lu, sequence %lu",
                        common->rpcId.clientId, common->rpcId.sequence);
                return;
            }
        ...
        switch (common->opcode) {
            ...
            case PacketOpcode::ALL_DATA: {
                ...
                NL_TRACE("server received ALL_DATA, clientId %lu, "
                        "sequence %lu, length %u",
                            header->common.rpcId.clientId,
                            header->common.rpcId.sequence, length);
            ...
            case PacketOpcode::DATA: {
                ...
                if (header == NULL)
                    goto packetLengthError;
                NL_TRACE("server received DATA, clientId %lu, sequence %lu, "
                        "offset %u, length %u",
                        header->common.rpcId.clientId,
                        header->common.rpcId.sequence,
                        header->offset, received->len);
                ...
                retainPacket = serverRpc->accumulator->addPacket(header,
                        received->len);
                if (header->offset == 0) {
                    NL_TRACE("server received opcode %u, totalLength %u",
            ...
            case PacketOpcode::GRANT: {
                ...
                if (header == NULL)
                    goto packetLengthError;
                NL_TRACE("server received GRANT, clientId %lu, sequence %lu, "
                        "offset %u",
                        header->common.rpcId.clientId,
                        header->common.rpcId.sequence,
                        header->offset);
            ...
            case PacketOpcode::RESEND: {
                ...
                if (header == NULL)
                    goto packetLengthError;
                NL_TRACE("server received RESEND, clientId %lu, sequence %lu, "
                        "offset %u, length %u",
                        header->common.rpcId.clientId,
                        header->common.rpcId.sequence,
                        header->offset, header->length);
                ...
                if (serverRpc == NULL) {
                    ...
                    NL_TRACE("server requesting restart, clientId %lu, "
                            "sequence %lu",
                            common->rpcId.clientId, common->rpcId.sequence);
            ...
            case PacketOpcode::ACK: {
                // Nothing to do.
                NL_TRACE("server received ACK, clientId %lu, sequence %lu",
                        common->rpcId.clientId, common->rpcId.sequence);
                return;
            }
            ...
            case PacketOpcode::ABORT: {
                NL_TRACE("server received ABORT, clientId %lu, sequence %lu",
                        common->rpcId.clientId, common->rpcId.sequence);
                if (serverRpc != NULL) {
...
BasicTransport::ServerRpc::sendReply()
{
    ...
    NL_TRACE("sendReply invoked, clientId %lu, sequence %lu, length %u, "
            "%lu outgoing responses", rpcId.clientId, rpcId.sequence,
    ...
    if (bytesSent > 0) {
        NL_TRACE("sendReply transmitted %u bytes", bytesSent);
    }
}
        ...
        uint8_t whoFrom)
{
    ...
    if (whoFrom == FROM_SERVER) {
        NL_TRACE("server requesting retransmission of bytes %u-%u, "
                 "clientId %lu, sequence %lu",
                    buffer->size(), endOffset,
                    rpcId.clientId, rpcId.sequence);
    } else {
        NL_TRACE("client requesting retransmission of bytes %u-%u, "
                 "clientId %lu, sequence %lu",
                    buffer->size(), endOffset,
                    rpcId.clientId, rpcId.sequence);
    }
...
#define MAX_PACKETS 8
    ...
    if (numPackets > 0) {
        ...
        NL_TRACE(startTime, "start of polling iteration %u, "
    ...
    for (ScheduledMessage* recipient : t->messagesToGrant) {
        ...
        if (whoFrom == FROM_CLIENT) {
            NL_TRACE("client sending GRANT, clientId %lu, "
                     "sequence %lu, offset %u",
                        recipient->rpcId.clientId,
                        recipient->rpcId.sequence,
                        grant.offset);
        } else {
            NL_TRACE("server sending GRANT, clientId %lu, "
                     "sequence %lu, offset %u",
                        recipient->rpcId.clientId,
                        recipient->rpcId.sequence,
                        grant.offset);
    ...
    if (now >= t->nextTimeoutCheck) {
        ...
        if ((numPackets < MAX_PACKETS)
            ...
            if (numPackets == MAX_PACKETS) {
                ...
                NL_TRACE("Deadline invocation of checkTimeouts");
            }
            t->checkTimeouts();
            result = 1;
    ...
    if (result) {
        NL_TRACE("end of polling iteration %lu, received %u packets, "
...
{
    NL_TRACE("checkTimeouts invoked, %lu client RPCs, %lu server RPCs",
    ...
    for (ClientRpcMap::iterator it = outgoingRpcs.begin();
        ...
        if (clientRpc->response->size() == 0) {
            ...
            if (clientRpc->silentIntervals % pingIntervals == 0) {
                NL_TRACE("client sending RESEND for clientId %lu, "
                        "sequence %lu", clientId, sequence);
