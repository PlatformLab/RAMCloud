...
#ifndef RAMCLOUD_MULTIREMOVE_H
#define RAMCLOUD_MULTIREMOVE_H
...
/**
 * This class implements the client side of multiRemove operations. It
 * uses the MultiOp Framework to manage multiple concurrent RPCs,
 * each removing one or more objects to a single server. The behavior
...
class MultiRemove : public MultiOp {
    static const WireFormat::MultiOp::OpType type =
                WireFormat::MultiOp::OpType::REMOVE;

  PUBLIC:
    MultiRemove(RamCloud* ramcloud,
  ...
  PROTECTED:
    ...
    DISALLOW_COPY_AND_ASSIGN(MultiRemove);
};

} // end RAMCloud
#endif /* RAMCLOUD_MULTIREMOVE_H */
