...
TEST_F(ClientTransactionManagerTest, addTransactionTask) {
    EXPECT_EQ(0U, txManager.taskList.size());
    EXPECT_FALSE(txManager.isRunning());
    txManager.addTransactionTask(taskPtr);
    EXPECT_EQ(1U, txManager.taskList.size());
    EXPECT_TRUE(txManager.isRunning());
}

TEST_F(ClientTransactionManagerTest, handleTimerEvent_basic) {
...
TEST_F(ClientTransactionManagerTest, handleTimerEvent_noSchedule) {
    txManager.taskList.push_back(taskPtrOther);
    taskPtrOther.get()->state = ClientTransactionTask::DONE;
