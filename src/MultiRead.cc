...
MultiRead::PartRpc::finish()
{
    ...
    uint32_t respOffset = downCast<uint32_t>(
    ...
    for (i = 0; i < reqHdr.count; i++) {
        ...
        if (*status == STATUS_OK) {
            const SegmentEntry* entry = response->getOffset<SegmentEntry>(
                    respOffset);
            if (entry == NULL) {
                TEST_LOG("missing segment entry");
                break;
            }
            respOffset += downCast<uint32_t>(sizeof(SegmentEntry));

            const Object* responseObject =
                    response->getOffset<Object>(respOffset);
            if (responseObject == NULL) {
                TEST_LOG("missing object header");
                break;
            }
            respOffset += downCast<uint32_t>(sizeof(Object));
            respOffset += responseObject->keyLength;

            request.version = responseObject->version;

            uint32_t dataLength = responseObject->dataLength(entry->length);
            if (response->getTotalLength() < respOffset + dataLength) {
                TEST_LOG("missing object data");
                break;
            }
            request.value->construct();
            response->copy(respOffset, dataLength,
                    new(request.value->get(), APPEND) char[dataLength]);
